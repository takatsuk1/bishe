...
func (g *CodeGenerator) generateWorker(req *AgentGenerateRequest) string {
	...
	buf.WriteString("\t\tresponse, err = w.agent.callTool(ctx, taskID, query, req.NodeConfig)\n")
	...
	buf.WriteString("func (a *Agent) callTool(ctx context.Context, taskID string, query string, nodeCfg map[string]any) (string, error) {\n")
