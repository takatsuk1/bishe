...
var ChatModelSystemPrompt = prompt.FromMessages(schema.FString,
	schema.SystemMessage(`你是一个智能助手，可以回答用户问题，帮助用户阅读、总结文章，并针对文章里的问题进行解答。
# 注意
1. 无论上下文是什么语言，使用中文输出。
2. 如果用户的问题与阅读、总结文章没有太大的关系，请返回结束语，并在最后加上<{stop_word}>结束任务，其余情况你可以提供一些相关话题引导用户追问。
3. 你只能用户的消息中提取URL，不能随意捏造URL进行访问。
...
func init() {
	_ = compose.RegisterSerializableType[*state]("urlreader:state")
	_ = compose.RegisterSerializableType[*Input]("urlreader:input")
	_ = compose.RegisterSerializableType[*Output]("urlreader:output")
}
...
func NewAgent() (*Agent, error) {
	...
	jinaCli, err := tools.NewJinaReader(ctx, &tools.JinaConfig{
		APIKey: cfg.Jina.APIKey,
	})
	if err != nil {
		return nil, fmt.Errorf("failed to create tavily client: %w", err)
	}
	a.jinaReader = jinaCli
...
func (a *Agent) createRunnableGraph(ctx context.Context, cfg *config.MainConfig) (compose.AnyGraph, error) {
	...
	extractTool, err := a.jinaReader.Tool()
	if err != nil {
		return nil, fmt.Errorf("failed to transfer jina reader tool: %w", err)
	}
	extractToolInfo, err := extractTool.Info(ctx)
	if err != nil {
		return nil, fmt.Errorf("failed to transfer jina reader tool: %w", err)
	}
	err = toolCallingModel.BindTools([]*schema.ToolInfo{extractToolInfo})
	if err != nil {
		return nil, fmt.Errorf("failed to transfer jina reader tool: %w", err)
	}
	...
	toolNode, err := compose.NewToolNode(ctx, &compose.ToolsNodeConfig{
		Tools: []tool.BaseTool{extractTool},
	})
	if err != nil {
...
func (cb *callbackHandler) OnEnd(ctx context.Context, info *callbacks.RunInfo,
	...
	switch info.Name {
	case "Tool:core":
		cb.updateWorkingTaskStatus(ctx, "> ✅文章内容获取完成\n\n")
	}
