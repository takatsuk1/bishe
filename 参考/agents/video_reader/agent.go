...
var ChatModelSystemPrompt = prompt.FromMessages(schema.FString,
	schema.SystemMessage(`你是一个智能助手，可以通过本地路径读取用户图片或者视频，根据用户的要求理解图片或者视频的内容，并作解答。
# 注意
1. 无论上下文是什么语言，使用中文输出。
2. 如果你认为已经解答了用户的问题，请返回结束语，并在最后加上<{stop_word}>结束任务，其余情况你可以提供一些相关话题引导用户追问。
3. 你只能用户的消息中提取文件路径，不能随意捏造路径进行访问。
4. 如果用户提供了本地文件的路径，你需要调用dashscope工具进行读取，而不是告诉用户你无法访问本地文件
...
type Agent struct {
	dashScope   *tools.DashScopeVLTool
...
func NewAgent() (*Agent, error) {
	...
	dashCli, err := tools.NewDashScopeVLTool(cfg.Dash.APIKey, cfg.LLM.VideoModel)
	if err != nil {
		return nil, fmt.Errorf("failed to create DashScope client: %w", err)
	}
	a.dashScope = dashCli
...
func (a *Agent) createRunnableGraph(ctx context.Context, cfg *config.MainConfig) (compose.AnyGraph, error) {
	...
	// 获取工具信息
	dashTool, err := a.dashScope.Tool()
	if err != nil {
		return nil, fmt.Errorf("failed to get dashscope tool info: %w", err)
	}

	dashToolInfo, err := dashTool.Info(ctx)
	if err != nil {
		return nil, fmt.Errorf("failed to get dashscope tool info: %w", err)
	}

	// 绑定工具到模型
	err = toolCallingModel.BindTools([]*schema.ToolInfo{dashToolInfo})
	if err != nil {
		return nil, fmt.Errorf("failed to bind dashscope tool: %w", err)
	}
	...
	toolNode, err := compose.NewToolNode(ctx, &compose.ToolsNodeConfig{
		Tools: []tool.BaseTool{dashTool},
	})
	if err != nil {
		return nil, fmt.Errorf("failed to create dashscope tool node: %w", err)
	}
...
func (cb *callbackHandler) OnEnd(ctx context.Context, info *callbacks.RunInfo,
	...
	switch info.Name {
	case "Tool:core":
		cb.updateWorkingTaskStatus(ctx, "> ✅视频内容读取完成\n\n")
	}
