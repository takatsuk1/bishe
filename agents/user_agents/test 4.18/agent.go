...
// Generated at: 2026-04-18T16:44:47+08:00
...
func NewAgent() (*Agent, error) {
	...
	if err := agent.orchestratorEngine.RegisterWorker(orchestrator.AgentDescriptor{
		ID:           Test418WorkflowWorkerID,
		Name:         "test 4.18 workflow worker",
		Capabilities: []orchestrator.AgentCapability{"chat_model", "tool", "test 4.18"},
	}, &workflowNodeWorker{agent: agent}); err != nil {
		return nil, err
	}

	wf, err := buildTest418Workflow()
	if err != nil {
		return nil, err
	}
...
func (a *Agent) ProcessInternal(ctx context.Context, taskID string, initialMsg internalproto.Message,
	...
	if err != nil {
		return fmt.Errorf("failed to wait test 4.18 workflow: %w", err)
	}
	if runResult.State != orchestrator.RunStateSucceeded {
		if runResult.ErrorMessage != "" {
			return fmt.Errorf("test 4.18 workflow failed: %s", runResult.ErrorMessage)
		}
		return fmt.Errorf("test 4.18 workflow failed")
	}
...
func (a *Agent) buildNodeProgressMessage(nodeID string, stepState internalproto.StepState) string {
	...
	nodeType := "node"
	if v := strings.TrimSpace(Test418NodeTypeByID[nodeID]); v != "" {
		nodeType = v
	}
	_ = stepState
		...
		for {
			...
			if err == nil {
				...
				if nodeID != "" && !started[nodeID] {
					...
					ev := internalproto.NewStepEvent("test 4.18", "workflow", nodeID, internalproto.StepStateStart, messageZh)
					text := messageZh
				...
				for _, nr := range run.NodeResults {
					...
					ev := internalproto.NewStepEvent("test 4.18", "workflow", id, stepState, messageZh)
					if token, tokenErr := internalproto.EncodeStepToken(ev); tokenErr == nil {
						text := messageZh + "\n" + token
						_ = manager.UpdateTaskState(ctx, taskID, internalproto.TaskStateWorking, &internalproto.Message{
							Role:  internalproto.MessageRoleAgent,
...
func buildTest418Workflow() (*orchestrator.Workflow, error) {
	wf, err := orchestrator.NewWorkflow(Test418WorkflowID, "test 4.18 default workflow")
	if err != nil {
		return nil, err
	}

	if err = wf.AddNode(orchestrator.Node{ID: "N1", Type: orchestrator.NodeTypeStart, Config: map[string]any{"input_source": "previous"}, Metadata: map[string]string{"next_to": "N_06d88b", "ui.agent": "orchestrator", "ui.label": "开始", "ui.x": "120", "ui.y": "120"}}); err != nil {
		return nil, err
	}
	if err = wf.AddNode(orchestrator.Node{ID: "N_06d88b", Type: orchestrator.NodeTypeChatModel, AgentID: Test418WorkflowWorkerID, TaskType: "chat_model", PreInput: "请基于用户问题，提取一个新的检索关键词。只输出关键词。", Config: map[string]any{"input_source": "previous", "input_type": "string", "output_type": "string"}, Metadata: map[string]string{"next_to": "N_25428c", "ui.agent": "custom", "ui.label": "模型推理", "ui.x": "376", "ui.y": "128"}}); err != nil {
		return nil, err
	}
	if err = wf.AddNode(orchestrator.Node{ID: "N_25428c", Type: orchestrator.NodeTypeTool, AgentID: Test418WorkflowWorkerID, TaskType: "test 4.18_default", Config: map[string]any{"input_mapping": map[string]any{"query": "N_06d88b"}, "input_source": "previous", "params": map[string]any{"api_key": "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"}, "tool_name": "tavily"}, Metadata: map[string]string{"next_to": "N_2ba7a0", "ui.agent": "custom", "ui.label": "工具调用", "ui.x": "623", "ui.y": "132"}}); err != nil {
		return nil, err
	}
	if err = wf.AddNode(orchestrator.Node{ID: "N_2ba7a0", Type: orchestrator.NodeTypeChatModel, AgentID: Test418WorkflowWorkerID, TaskType: "chat_model", PreInput: "根据当前检索结果整理输出为结构化内容。当前结果: ", Config: map[string]any{"input_source": "previous", "input_type": "string", "output_type": "string"}, Metadata: map[string]string{"next_to": "N_533574", "ui.agent": "custom", "ui.label": "模型推理", "ui.x": "862", "ui.y": "131"}}); err != nil {
		return nil, err
	}
	if err = wf.AddNode(orchestrator.Node{ID: "N_533574", Type: orchestrator.NodeTypeEnd, Config: map[string]any{"input_source": "previous"}, Metadata: map[string]string{"next_to": "", "ui.agent": "orchestrator", "ui.label": "结束", "ui.x": "869", "ui.y": "365"}}); err != nil {
		return nil, err
	}
	if err = wf.AddEdgeWithLabel("N1", "N_06d88b", "", nil); err != nil {
		return nil, err
	}
	if err = wf.AddEdgeWithLabel("N_06d88b", "N_25428c", "", nil); err != nil {
		return nil, err
	}
	if err = wf.AddEdgeWithLabel("N_25428c", "N_2ba7a0", "", nil); err != nil {
		return nil, err
	}
	if err = wf.AddEdgeWithLabel("N_2ba7a0", "N_533574", "", nil); err != nil {
		return nil, err
	}
	return wf, nil
}
